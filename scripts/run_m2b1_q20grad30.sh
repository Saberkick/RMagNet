...
RUN_DIR="${RUN_DIR:-$PROJECT/runs/m2_corrected_b1_q20grad30}"
...
if [[ "$(awk -F, '{print NF}' <<<"$CUDA_VISIBLE_DEVICES")" -ne 4 ]]; then
  echo "M2-B1 requires exactly four visible GPUs" >&2
  exit 1
fi
...
"$UV_BIN" run --no-project --python "$ENVIRONMENT/bin/python" \
  python -m rmagnet.m2b1_q20 --preflight-only
...
exec "$UV_BIN" run --no-project --python "$ENVIRONMENT/bin/python" \
  torchrun --standalone --nproc-per-node=4 -m rmagnet.m2b1_q20 \
  ...
  --validate-every 35 --save-every 35 \
