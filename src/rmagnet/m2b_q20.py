"""M2-B: strict 70-step M2 comparison with the complete C1 Q20 supervision."""
...
DEFAULT_RUN = PROJECT / "runs/m2_corrected_b_q20full70"
...
def main() -> None:
    ...
    preflight = {
        "status": "preflight_ok",
        "experiment": "M2-B-corrected-q20full70",
    ...
    if world_size() != 4 or args.batch_size != 1 or args.gradient_accumulation != 1:
        raise RuntimeError("M2-B strict comparison requires 4 GPUs, batch 1, accumulation 1")
    if args.max_steps != 70 or args.validate_every != 35 or args.save_every != 35:
        raise RuntimeError("M2-B strict comparison requires 70 steps and checkpoints at 35/70")
    ...
    if is_main:
        ...
        metadata = {
            ...
            "q20_gradient_control": "epoch 1 ramp to 15%; epoch 2 target 20%; measure every 20 updates; EMA; lambda [0.02,0.5]",
    ...
    for epoch in range(args.epochs):
        ...
        for batch in loader:
            ...
            if upcoming_step == 1 or upcoming_step % args.gradient_measure_every == 0:
                ...
                target_ratio = 0.15 * min(1.0, upcoming_step / len(loader)) if epoch == 0 else 0.20
                desired = target_ratio * measured_base_norm / measured_q_norm
                desired = min(args.lambda_q_max, max(args.lambda_q_min, desired))
                lambda_q = args.lambda_q_ema * lambda_q + (1.0 - args.lambda_q_ema) * desired
    ...
    if global_step != 70:
        raise RuntimeError(f"M2-B stopped at {global_step}, expected 70")
    dist.barrier()
    if is_main:
        ...
        summary = {
            "status": "complete", "experiment": "M2-B-corrected-q20full70", "optimizer_updates": 70,
